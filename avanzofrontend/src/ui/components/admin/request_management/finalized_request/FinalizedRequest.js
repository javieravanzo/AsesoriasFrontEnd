...
//Actions
import {getAllFinalizedRequest} from "../../../../../store/redux/actions/admin/adminActions";

//Styles
import '../../../../styles/admin/request_management/request-state.css';
import { SUCCESS_MODAL } from '../../../subcomponents/modalMessages';

class FinalizedRequest extends Component {
  ...
  constructor (props) {
    ...
    this.props.getAllFinalizedRequest();
  ...
  render(){
    ...
    let {idRequest, identificationId, requestStateName, quantity, createdAt} = this.state;
    let tableData = this.setData(this.props.finalizedRequest);
    //console.log("JSON", tableData, JSON.stringify(tableData));
    
    if(this.props.finalizedRequest === null){
...
FinalizedRequest.propTypes = {
  finalizedRequest: PropTypes.array,
};

const mapStateToProps = (state) => {
  return {
    finalizedRequest: state.admin.finalizedRequest,
  }
};

const mapDispatchToProps = (dispatch) => {
  return {
    getAllFinalizedRequest: () => dispatch(getAllFinalizedRequest()),
  }
};

export default connect(mapStateToProps, mapDispatchToProps)(FinalizedRequest);
